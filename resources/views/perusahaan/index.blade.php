          ...
          <table class="min-w-full rounded-2xl overflow-hidden border border-[#7096D1]">
            <thead class="bg-[#334EAC] text-[#FFF9F0]">
              <tr>
                ...
                <th class="py-2 px-3 border border-[#7096D1]">Kontak</th>
            ...
            <tbody>
              @foreach($data as $i => $p)
                <tr class="hover:bg-[#FFF9F0] even:bg-[#f3f6fc] transition">
                  ...
                  <td class="py-2 px-3 border border-[#7096D1]">{{ $p->nama }}</td>
                  <td class="py-2 px-3 border border-[#7096D1]">{{ $p->alamat ?? '-' }}</td>
                  <td class="py-2 px-3 border border-[#7096D1]">{{ $p->pic ?? '-' }}</td>
                  <td class="py-2 px-3 border border-[#7096D1]">{{ $p->kontak ?? '-' }}</td>
                  <td class="py-2 px-3 border border-[#7096D1]">{{ $p->owner->email ?? '-' }}</td>
                  <td class="py-2 px-3 border border-[#7096D1]">12345678</td>
                  <td class="py-2 px-3 border border-[#7096D1]">
                    <div class="flex items-center justify-center gap-2">
                      <a href="{{ route('perusahaan.edit', $p) }}"
                         class="bg-[#7096D1] text-white px-3 py-1 rounded-lg text-sm hover:bg-[#334EAC] transition">
                        Edit
                      </a>
                      <form action="{{ route('perusahaan.destroy', $p) }}" method="POST" style="display:inline"
